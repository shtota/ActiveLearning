...
from feature_extraction import ALL_ENCODERS, ENCODER_NAME_TO_CLASS
...
class Parser:
    ...
    def parse_args():
        ...
        return names, encoders, strategy, args.round, models, float(args.downsample), args.vectors, args.batch, args.skip


def main():
    names, encoders, strategies, start_round, models, downsample, vecs, batch_size, skip_existing = Parser.parse_args()
    a = time.time()
    for encoder_name in encoders:
        for dataset_name in names:
            encoder = ENCODER_NAME_TO_CLASS[encoder_name]()
            dataset_name, X_train, X_test, y_train, y_test = load_train_test(dataset_name, encoder, downsample)
            encoder.clear()

            learner = ActiveLearner(X_train, y_train, X_test, y_test, dataset_name, encoder_name, vecs, batch_size, skip_existing)
